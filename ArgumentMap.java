...
/**
 * 
 * @author SirIan This will check for me the paths of the arguments and stores
 *         them in a TreeMap.
 *
 */
public class ArgumentMap {
	...
	public static boolean isValue(String arg) {
		return (arg != null && !arg.trim().isEmpty() && arg.trim().length() >= 1 && !arg.trim().startsWith("-"));
	}
	 ...
	 */
	public Path getPath(String flag) throws java.nio.file.InvalidPathException {
		try {
			return Paths.get(map.get(flag)) == null ? null : Paths.get(map.get(flag));
	 ...
	 */
	public Path getPath(String flag, Path defaultValue) throws NullPointerException {
		return map.get(flag) == null ? defaultValue : Paths.get(map.get(flag));
	}
	...
	/**
	 * This will check if the path is a directory or not given the path
	 * 
	 * @param path the path to see if it is a directory or not
	 * @return true if it is a directory or False.
	 */
	public boolean isDirective(Path path) {
		return Files.isDirectory(path);
	}

	/**
	 * Returns the argument map as a string
	 */
	@Override
